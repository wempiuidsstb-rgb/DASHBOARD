  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bento-card bg-white border-2 border-slate-300 p-6 sm:p-7 shadow-sm">
        <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-lg bg-blue-100 border border-blue-300 text-blue-950 text-xs sm:text-sm font-black uppercase tracking-wide mb-3">
          <Sparkles className="w-4 h-4 text-blue-800" />
          <span className="text-blue-950 font-black">Analisa Strategis Pemeliharaan Right-of-Way (ROW)</span>
        </div>
        <h3 className="text-xl sm:text-2xl font-black text-slate-950 tracking-tight">
          Ringkasan Eksekutif & Temuan Kunci Operasional UP3 Bulukumba
        </h3>
        <p className="text-sm sm:text-base text-slate-900 font-semibold mt-3 max-w-3xl leading-relaxed">
          Berdasarkan analisis terhadap <span className="font-black text-slate-950">{totalGiat.toLocaleString('id-ID')}</span> log aktivitas pemeliharaan ROW,
          telah direalisasikan pembersihan jalur sepanjang{' '}
          <strong className="font-black text-slate-950 bg-slate-100 px-2 py-0.5 rounded border border-slate-300">
            {totalKms.toLocaleString('id-ID', { minimumFractionDigits: 1, maximumFractionDigits: 1 })} KMS
          </strong>{' '}
          dan penebangan sebanyak{' '}
          <strong className="font-black text-slate-950 bg-amber-100 px-2 py-0.5 rounded border border-amber-300">
            {totalBtg.toLocaleString('id-ID')} Batang
          </strong>{' '}
          pohon rawan roboh yang berpotensi menyebabkan gangguan transmisi/distribusi SUTM.
        </p>

        {/* 4 Quick Strategic Highlights */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3.5 mt-6">
          <div className="bg-slate-100 p-4 rounded-xl border border-slate-300 shadow-2xs">
            ...
            <div className="text-2xl font-black text-slate-950 mt-1">
              {tebangPerKmsRatio} <span className="text-xs font-bold text-slate-700">btg/KMS</span>
            </div>
          ...
          <div className="bg-slate-100 p-4 rounded-xl border border-slate-300 shadow-2xs">
            ...
            <div className="text-2xl font-black text-slate-950 mt-1 truncate">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bento-card">
                  ...
                  return (
                    ...
                    >
                      <td className="py-2 px-3 font-bold text-slate-900 flex items-center gap-2">
                        <span className="w-4 h-4 rounded-full bg-slate-200 text-slate-800 flex items-center justify-center text-[10px] font-mono font-bold">
                          {idx + 1}
                        </span>
                        {u.ulp.replace('ULP ', '')}
                      </td>
          ...
          <div className="mt-3.5 p-3 bg-slate-50 rounded-xl text-xs text-slate-700 flex items-start gap-2 border border-slate-200">
            <Info className="w-4 h-4 text-blue-600 shrink-0 mt-0.5" />
            <p>
              <strong className="text-slate-900">Insight:</strong> ULP dengan rasio Tebang/KMS tinggi menunjukkan vegetasi pohon kayu keras
        ...
        <div className="bento-card">
          ...
          <div className="space-y-3 mt-4">
            {timEfficiency.map((t) => (
              <div
                ...
                className="p-3.5 rounded-xl border border-slate-200 hover:border-slate-400 hover:bg-slate-50 cursor-pointer transition-all space-y-2"
              >
                <div className="flex items-center justify-between">
                  <span className="text-xs font-bold text-slate-900">{t.tim}</span>
                  <span className="text-xs font-bold text-slate-900 bg-slate-100 px-2 py-0.5 rounded-md border border-slate-300">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 text-xs pt-1">
                  <div className="bg-slate-50 p-2 rounded-lg border border-slate-100">
                    <span className="text-slate-500 text-[10px] font-semibold block">Rampal/Giat</span>
                    <span className="font-extrabold text-slate-900">{t.kmsPerGiat} KMS</span>
                  </div>
                  <div className="bg-slate-50 p-2 rounded-lg border border-slate-100">
                    <span className="text-slate-500 text-[10px] font-semibold block">Tebang/Giat</span>
                    <span className="font-extrabold text-slate-900">{t.btgPerGiat} btg</span>
                  </div>
                  <div className="bg-slate-50 p-2 rounded-lg border border-slate-100 col-span-2 sm:col-span-1">
                    <span className="text-slate-500 text-[10px] font-semibold block">Total Realisasi</span>
                    <span className="font-extrabold text-slate-900">{t.kms.toLocaleString()} KMS</span>
                  </div>
                </div>
              </div>
            ))}
          ...
          <div className="mt-3.5 p-3 bg-slate-50 rounded-xl text-xs text-slate-700 flex items-start gap-2 border border-slate-200">
            ...
            <p>
              <strong className="text-slate-900">Karakteristik:</strong> Regu <strong>YANTEK</strong> menjadi tulang punggung volume kegiatan terbanyak
      ...
      <div className="bento-card bg-white border-2 border-slate-300 p-6 sm:p-7 shadow-sm">
        <div className="flex items-center gap-2 text-slate-950 font-black text-base mb-4">
          <Lightbulb className="w-5 h-5 text-amber-600" />
          <span>Rekomendasi Strategis & Rencana Aksi Preventif PLN UP3 Bulukumba</span>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="bg-slate-100 p-4 rounded-xl border border-slate-300 shadow-2xs space-y-2">
            <div className="flex items-center gap-2 text-xs font-black text-slate-950">
              <span className="w-5 h-5 rounded-full bg-slate-950 text-white flex items-center justify-center text-[11px] font-bold">
                1
              </span>
